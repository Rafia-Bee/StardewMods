...
/// Routes generated postings to the right vanilla delivery mechanism. Daily-board postings and mail
/// postings both add a custom Quest to the player's journal and queue a mail letter for flavor;
/// special-orders and NPC-dialogue paths are stubbed for later phases.
internal sealed class QuestPoster
{
    ...
    {
        var quest = BuildVanillaQuest(posting);
        if (quest == null)
        {
        ...
        _monitor.Log($"Posted {posting.DefinitionId} via {posting.Kind}. Days left: {quest.daysLeft.Value}.", LogLevel.Trace);
    }
    ...
    private static Quest? BuildVanillaQuest(QuestPosting p)
    {
        ...
        switch (p.QuestType)
        {
            ...
            {
                // SocializeQuest randomizes its own target list; not safe to inject. Use a fallback ItemDelivery.
    ...
    {
        // Mail format: text ^ for line break, %item quest <questId> %% to attach quest.
        // We've already added the quest directly; this letter is purely flavor.
