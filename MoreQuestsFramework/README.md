...
- **Quest factory.** Builds the right vanilla `Quest` subclass for each posting (`ItemDeliveryQuest`, `FishingQuest`, `SlayMonsterQuest`, `ResourceCollectionQuest`, plus the framework's own multi-step `AdventureQuest` and shipping-tracked `MoreQuestsShipQuest`) and stamps a unique ID so external trackers attribute the quest correctly.
...
- **Custom Quest subclasses.** `MoreQuestsItemDeliveryQuest` and `MoreQuestsFishingQuest` implement `IRewardedQuest` (a `serializedRewards` NetStringList that survives save round-trip) and override the vanilla turn-in logic to actually consume the requested items (vanilla's `FishingQuest.OnNpcSocialized` doesn't reduce the stack — you could sell every fish and still claim the reward). `MoreQuestsShipQuest` is a single-objective shipping quest observed at `DayEnding` against `Game1.getFarm().getShippingBin(player)`; vanilla still sells the items at full price, the framework only counts matches. `AdventureQuest` adds a multi-step substrate: each step ships its own kind (`Deliver`, `Talk`, `Gift`, `Ship`, `Catch`, `Slay` so far; `Visit`, `Build`, `ReachLevel`, `Plant`, `Collect`, `ClearDebris`, `ClearWeeds`, `Custom` in later phases) and `Requires[]` ordering; every active step (not-Done with prerequisites met) sees each event in parallel, so independent steps can be completed in any order. All step kinds dispatch through vanilla `Quest` virtuals (`OnItemOfferedToNpc`, `OnNpcSocialized`, `OnFishCaught`, `OnMonsterSlain`) — no Harmony patches.
...
Adventure (multi-step) quests use `Steps[]` instead of `Objective`. Each step has a `Name` (used by other steps' `Requires[]`), a `Kind`, a `Description` shown in the journal, and step-kind-specific targeting fields. `$giver` in `Targets[]` rewrites to the resolved giver name at quest-creation time. Step kinds available now: `Deliver`, `Talk`, `Gift`, `Ship`, `Catch`, `Slay`. Remaining kinds (`Visit`, `Build`, `ReachLevel`, `Plant`, `Collect`, `ClearDebris`, `ClearWeeds`, `Custom`) land in later sub-phases. Independent steps (no `Requires[]`) are all active simultaneously and may be completed in any order; the journal renders one bullet per active step.

Single-objective `Ship` quests use `"Objective": { "Kind": "Ship", "Item": "(O)787", "Count": 1 }`. The `Item` field accepts either a single string or an array of strings — when an array is supplied, the quest accepts any id in the set as a satisfying delivery. The framework observes `Game1.getFarm().getShippingBin(player)` at `DayEnding`; vanilla still sells the items at full price, the framework only counts matches.

`MailReward` accepts `"When": "Today"` (default), `"Tomorrow"`, or `"NextDay"` (alias for `Tomorrow`). `NextDay` writes to `Game1.player.mailForTomorrow` so the letter shows up in the next morning's mailbox.
