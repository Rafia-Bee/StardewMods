...
- **Trigger sources.**
  ...
  - `CustomBoard`, a per-day draw routed to one specific board by its id. The quest names its board with `Trigger.CustomBoardId`; leave it out and the quest falls back to its owner's single board. Capped at the board's `PoolSize`, with `AllowedCategories` as an optional extra filter. See the Custom boards section for how routing and catch-all boards work.
...
- **Timed deliveries (in-game countdowns).** A C# generator can mark one Deliver step on an `AdventureQuest` as a timed package run via `ConfigureTimedDelivery(...)`. When the Talk step gating it completes, the framework hands the player the package item, picks a target villager who is reachable right now (not the giver, not in the giver's building, not invisible, and only somewhere the player has visited), rolls a limit between the given min and max in-game minutes, and counts down on the game clock, so menus, events, and time mods all behave. The giver refuses the handoff (and the step stays open) on festival days, past the configured cutoff hour, when the worst-case roll wouldn't fit before 2am, with a full inventory, or with nobody to deliver to; the refusal line is spoken once per day and never blocks gifting or normal dialogue. The countdown shows in the vanilla journal objective and through `GetAdventureSteps`, so a HUD that re-reads it (like Quest Journal's pinned tracker) ticks down live. Running out of time, sleeping, or cancelling after the pickup pulls the package back and costs the configured friendship with both NPCs. C# only for now, there is no JSON field for it yet.
- **Custom boards.** `BoardDefinition` registers a per-location pin-board at a tile of your choosing. `LoadBoardsFromMod(helper, "assets/boards.json")` auto-loads a JSON pack. Quests with `Trigger.Source: "CustomBoard"` route to one board by id each `DayStarted`. A quest names its board with `Trigger.CustomBoardId`: a bare name resolves under the quest's owner, `"Owner.UniqueID/Name"` points at another mod's board. Leave it out and the quest goes to its owner's single board; if the owner has zero or more than one board and sets no id, the quest is dropped and you get one clear warning at startup. Routing is by id alone, so one mod's quests can never land on another mod's board by accident, a quest only crosses mods when it explicitly names that mod's board. `AllowedCategories` is now just an optional within-board filter, not what keeps boards apart. A board can opt in as a catch-all with `AllowedOwners` (`["*"]` mirrors every owner's quests, or list specific UniqueIDs); a catch-all mirrors live postings up to its own `PoolSize`, picked with its `PoolStrategy`, and a mirrored quest is one shared slot so accepting it on either board clears it from both. The framework renders the board sprite plus a bobbing "!" indicator in-world, opens a cork-board `CustomBoardMenu` on action-button click, and reuses vanilla `Billboard(true)` as the inner accept-quest popup. A board also takes a player-adjustable pin count, a `Layout` (scatter, tilted grid, or zoned regions), per-category pad/pin textures and corner icons, and optional category/poster priority, see the Custom board layout section.
- **Runtime trigger-source overrides.** `IMoreQuestsModApi.OverrideTriggerSource(definitionId, source)` re-routes an already-registered quest to a different `TriggerSource` without re-registration. Useful for content-mod toggles that flip a quest between the help-wanted board and a custom board (e.g. guild board on -> off, falls the guild quests back to the help-wanted board so they stay reachable). There's a three-argument overload `OverrideTriggerSource(definitionId, source, board)` that also names the target board, for routing a quest that has no `Trigger.CustomBoardId` of its own (such as a framework-owned quest) onto your board. A bare board name resolves under your mod's UniqueID. The pipeline checks the override before reading `def.Source`, so the flip takes effect after you go to sleep. Overrides called during the registration window that target a quest its owner hasn't registered yet are buffered and replayed when the owner registers, so load order between consumer mods doesn't matter as long as both register before `RegistrationClosed`.
...
- `PadTexture` and `PinTexture` (optional), asset names for this category's own note paper and tack sprite. When set, every note in this category draws these instead of the default (or the board's) pad/pin. Still tinted by `PadColor` / `PinColor`, so paint them light if you want the tint to show. Keep them square like the default sprites. Missing falls back to the board's texture, then the framework default.
- `Icon` (optional), the little picture in the note corner. `"Portrait"` (or omitted) draws the quest giver's face, the default look; `"None"` draws nothing (an anonymous note); anything else is an asset name to draw instead (for example a "!" sprite). `IconSource` (optional `[x, y, w, h]`) picks a region of that texture. `IconScale` (optional, 0-1) sets the size as a fraction of the note (default `0.28`). `IconAnchor` places it in a corner: `BottomLeft` (default), `BottomRight`, `TopLeft`, `TopRight`, or `Center`. For exact placement, set `IconX` and `IconY` (both 0-1, from the top-left) instead of an anchor.

A single quest can override just its icon with a top-level `"Icon"` field on its quest entry (same values: `Portrait` / `None` / an asset name), which beats whatever its category sets. The icon's size and position still come from the category.
...
This swaps the pad/pin for the whole game (every board, every note). For a per-category sprite (so one category's notes look different from another's), use the `PadTexture` / `PinTexture` fields in the Categories section above instead.

## Custom board layout, pin count, and prioritization

On top of the routing fields, a board in the `.../Boards` asset takes these:

**Pin count (player-adjustable).** `PoolSize` is the default number of quests pinned up. Add `PoolSizeMin` and `PoolSizeMax` to give players a slider for this board in GMCM (under "Custom boards"). The slider runs Min to Max and starts at `PoolSize`; `PoolSizeMin` is also the floor, so the board never shows fewer than that even at the bottom of the slider. Defaults are 1 and 10. A board only fills up if enough quests qualify that day, so an early-game board with few eligible quests naturally looks sparse, that's not a bug.

**Layout.** `Layout` picks how the notes are arranged:

- `Scatter` (default) the loose, randomly pinned look custom boards have always used.
- `TiltedGrid` the tidy auto-sized grid with gentle per-note tilts (the look the daily help-wanted board uses).
- `Zoned` carve the board into regions, each with its own categories and sub-layout (below).

**Zones.** With `Layout: "Zoned"`, list `Zones`. Each zone has a `Rect` of `[x, y, width, height]` as percentages of the board's note area, measured from the top-left corner; a `Style` of `TiltedGrid` or `Scatter`; and a `Categories` list naming which quest categories live there. The first zone whose `Categories` match a note wins it. A zone with no `Categories` (or an empty list) is the catch-all for anything no other zone claimed; with no catch-all, an unclaimed note is just left off the board. Each zone sizes its own notes to fit its rectangle, so a near-empty zone shows big notes and a full one shrinks them. The example below puts Mining quests in a tilted grid across the top-left (60% wide, 50% tall) and Social quests scattered in the top-right.

```jsonc
"Layout": "Zoned",
"Zones": [
    { "Rect": [0, 0, 60, 50], "Style": "TiltedGrid", "Categories": ["Mining"] },
    { "Rect": [61, 0, 39, 40], "Style": "Scatter", "Categories": ["Social"] }
]
```

**Prioritization.** When more quests qualify than there are pins, the board picks by each quest's weight (the same weighted draw as always). To bias that, add a `Priority` block of per-category and/or per-poster multipliers. `2.0` makes a category twice as likely to win a pin, `0.5` half as likely, `0` shuts it out. Anything not listed stays at `1.0`, so a board with no `Priority` behaves exactly like before, and you can leave it off entirely.

```jsonc
"Priority": {
    "Categories": { "Mining": 2.0, "Social": 0.5 },
    "Owners": { "Some.OtherModId": 1.5 }
}
```
...
- `PinDays` keep the notice up for this many days once it's drawn. While pinned it shows every day without being re-rolled, so other notices can't bump it off the board. 0 (default) means it isn't pinned.
