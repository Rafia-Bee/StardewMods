...
### Mail Services Mod specifically

When Mail Services Mod is installed, the framework handles mailbox turn-ins for its own quests itself (a higher-priority prefix on `GameLocation.mailbox`). MSM's quest shortcut pays a hardcoded full heart of friendship on top of the quest's own rewards and skips quality, alternative-item and partial-stack rules, so the framework steps in front: the quest's declared rewards are the only payout, MSM's configured delivery fee is still charged, and quality/alternates/partial stacks behave exactly like an in-person hand-in. This covers both the framework's delivery quests and `AdventureQuest` deliver steps (which MSM can't see at all, since `AdventureQuest` isn't an `ItemDeliveryQuest`). Vanilla delivery quests are left to MSM.

Mailed gifts count too: the framework postfixes `NPC.receiveGift`, so `Gift`/`GiftUniqueNpcs` steps tick whether the gift is handed over in person or sent through MSM's gift service (or any other mod that calls `receiveGift` directly). A gift only ever counts once, so in-person gifts that already ticked through the vanilla hook aren't counted twice.
