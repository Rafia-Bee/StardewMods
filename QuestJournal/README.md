...
- **Special Orders too.** The Special Orders board quests show up on their own tab with their rewards itemised. You can also cancel one you don't want, which the game normally won't let you do. Anything you donated to it comes back in the lost and found box at the desk.
...
- **Spot finished quests fast.** In tabs that show both finished and active quests (the All tab and your custom ones), completed quests show their name in green so they don't blend in with the ones you're still doing. If you'd rather not see them there at all, there's a setting to keep completed quests in the Completed tab only.
...
- Turn on **"Replace the vanilla quest log"** in the settings and the game's own quest button opens this journal instead. (Off by default, and skipped if you use Deluxe Journal, since that already takes over the quest button.)
